...
class HunYuanRotaryEmbedding(nn.Module):
    def __init__(
        ...
        scaling_factor=1.0,
        rope_type="default",
        config: Optional[HunYuanConfig] = None,
    ):
        super().__init__()
        ...
        if config is None:
            logger.warning_once(
                "`HunYuanRotaryEmbedding` can now be fully parameterized by passing the model config through the "
...
class HunYuanAttention(nn.Module):
    ...
    def __init__(self, config: HunYuanConfig, layer_idx: Optional[int] = None):
        ...
        self.q_layernorm = HunYuanRMSNorm(128, eps=config.rms_norm_eps)
        self.k_layernorm = HunYuanRMSNorm(128, eps=config.rms_norm_eps)
...
class HunYuanSdpaAttention(HunYuanAttention):
    ...
    `HunYuanAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "HunYuanModel is using HunYuanSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class HunYuanDecoderLayer(nn.Module):
    def __init__(self, config: HunYuanConfig, layer_idx: int):
        ...
        # self.self_attn = HunYuan_ATTENTION_CLASSES[config._attn_implementatio](config=config, layer_idx=layer_idx)
        self.self_attn = HunYuan_ATTENTION_CLASSES['sdpa'](config=config, layer_idx=layer_idx)

        self.mlp = HunYuanMLP(config)
...
class HunYuanForCausalLM(HunYuanPreTrainedModel, GenerationMixin):
